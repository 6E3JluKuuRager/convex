...
class TestVoid:
    ...
    # Минимальная длина нульугольника нулевая
    def test_min_dlin(self):
        assert self.f.min_dlin() == 0.0
...
class TestPoint:
    ...
    # Минимальная длина одноугольника нулевая
    def test_min_dlin(self):
        assert self.f.min_dlin() == 0.0
...
class TestSegment:
    ...
    # Минимальная длина двуугольника равен длине отрезка
    def test_min_dlin(self):
        assert self.f.min_dlin() == approx(1.0)
...
class TestPolygon:
    ...
    # Изменение минимальной длины многоугольника
    # изначально она равна
    def test_min_dlin1(self):
        assert self.f.min_dlin() == approx(1.0)
    # добавление точки может ее изменить

    def test_min_dlin2(self):
        assert self.f.add(R2Point(-3.0, -3.0)).min_dlin() == approx(sqrt(2))
    # добавление новой точки может ее не изменить

    def test_min_dlin3(self):
        assert self.f.add(R2Point(10.0, 0.0)).min_dlin() == approx(1.0)
